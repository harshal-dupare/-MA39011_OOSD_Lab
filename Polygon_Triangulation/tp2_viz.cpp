...
void decompose_polygon(polygon &p, node *temp1[], node *temp2[], int &at, int t[][2])
{
    ...
    // {
    //     arr[i].nd->p->show();
    //     cout << ": ptype : " << ptypec[arr[i].nd->p->ptype];
    //     cout << endl;
    // }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else if (arr[i].nd->p->ptype == regularv)
        {
            ...
            if (arr[i].next->p->y > arr[i].nd->p->y)
            {
                ...
                // ##################### uncomment
                // edge* topedge  = edge_tree.get_topedge(&arr[i]); // later we can replace leftedge with top edge

                if (topedge->helper->p->ptype == mergev)
                {
                    OK("left edge helper ok")
                    // we connect only if the helper vertex is merge vertex
        ...
        edge_tree.in_order_printing(edge_tree.root);
        cout << endl;
        cout << endl;
        cout << endl;
    }
